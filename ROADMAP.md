...
## v0.3 — dictation, and the model lifecycle

- [x] A dictation dictionary sent as the model's vocabulary hint, and a way to
      keep dictations out of history
- [x] Press-to-start, press-to-stop dictation with a configurable recording
      limit
- [x] Fn as a push-to-talk key, watched as a physical key through IOHIDManager
      because macOS synthesizes fn-flagged events around every arrow and
      navigation key (hands-on verification on real keyboards still due)
- [x] Spoken "new line" and "new paragraph", replacements, and tidy whitespace
      for dictated text
- [x] Launch at login
- [x] First-run download progress, the dictation model loaded at start, idle
      models released
- [x] The server as a package, the dependency lock generated by uv, the macOS
      CI job installing and testing the real runtime, a release dry run

## v0.4 — broader workflow support
